        ...
        >
          {HOMEPAGE_SERVICES.map((service) => (
            <motion.div key={service.title} variants={item}>
              <Link
                href={service.href}
                className="block glass-dark rounded-2xl gradient-border overflow-hidden no-underline hover:glow-md transition-all duration-300 hover:-translate-y-2 group"
              >
                ...
                <div className="p-6">
                  <h3 className="text-lg font-bold text-gradient-gold mb-2">{service.title}</h3>
                  <p className="text-text-on-dark-muted text-sm leading-relaxed mb-4">
                    {service.description}
                  </p>
                  <span className="text-gold text-sm font-bold inline-flex items-center gap-1 group-hover:gap-2 transition-all">
