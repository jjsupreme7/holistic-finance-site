          ...
          {cards.map((c) => (
            <Link key={c.title} href={c.href} className="no-underline block group">
              <motion.div
                ...
                className="glass rounded-3xl p-9 text-center gradient-border glow-sm relative overflow-hidden hover:glow-md transition-all"
              >
                <span className={`absolute top-4 right-4 text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wider ${c.badgeColor}`}>
                  {c.badge}
                </span>
                <div className="w-16 h-16 mx-auto mb-5 rounded-2xl bg-gradient-to-br from-gold/15 to-gold/5 flex items-center justify-center text-gold-dark">
                  <Icon name={c.icon} size={28} />
                </div>
                <h3 className="text-xl font-bold text-dark mb-3">{c.title}</h3>
                <p className="text-text-light leading-relaxed mb-4">{c.description}</p>
